...
def cross_power_spectra_pylians(boxsize, sim_type, spec, ngrid_min, ngrid_max, ngrid_step, bulk_species1, bulk_species2, file_path, mass_limit, save_path):
    """
    ...
    Args:
        ...
        bulk_species1 (str): Type of bulk species (e.g., 'cdm', 'halo', 'nu').
        bulk_species2 (str): Type of bulk species (e.g., 'cdm', 'halo', 'nu').
    ...
    loop_spectra_computation(rank, boxsize, sim_type, spec, ngrid_min, ngrid_max, ngrid_step, size, bulk_species1, bulk_species2, file_path, mass_limit, save_path)
    comm.Barrier()
...
def loop_spectra_computation(rank, boxsize, sim_type, spec, ngrid_min, ngrid_max, ngrid_step, size, bulk_species1, bulk_species2, file_path, mass_limit, save_path):
    """
    ...
    for ngrid in ngrid_list_split[rank]:
        ...
        power_data = {}
        compute_cross_spectra(rank, file_path, bulk_species1, bulk_species2, spec, ngrid, sim_type, boxsize, power_data, mass_limit)
        simulation = simulation_def(boxsize, spec, sim_type, bulk_species1, bulk_species2, mass_limit)
...
def check_simulation_errors(sim_type, bulk_species1, bulk_species2, rank, comm):
    if sim_type == "0.0ev" and (bulk_species1 == "nu" or bulk_species2 == "nu") and rank == 0:
        print("In the case of LCDM we don't have nu snapshots!", flush=True)
        comm.Abort(1)


# Function to process halo data
def compute_cross_spectra(rank, file_path, bulk_species1, bulk_species2, spec, ngrid, sim, boxsize, power_data, mass_cut = 1.0):
    if rank == 0:
    ...
    data = load_files_pickles(file_path, bulk_species1, spec, ngrid, sim, mass_cut)
    data2 = load_files_pickles(file_path, bulk_species2, spec, ngrid, sim, mass_cut)
...
def simulation_def(boxsize, spec, sim_type, species1, species2, mass_limit):
    if species1 == "halo":
        return sim_type+'_'+spec+'_'+species1+f'_mass_{mass_limit:.1e}'+"_"+species2
    elif species2 == "halo":
        return sim_type+'_'+spec+'_'+species2+f'_mass_{mass_limit:.1e}'+"_"+species1
    else:
        return sim_type+'_'+spec+'_'+species1+"_"+species2        
...
def save_dataframe(power_data, simulation, ngrid, save_path):
    ...
    with open(save_path+'/Cross_spectra_ngrid_'+str(ngrid)+'_sim_'+simulation+'.pickle', 'wb') as handle:
