...
## Global variables
#################
sims= ["0.0ev", "0.2ev", "0.4ev"];
specs = ["L_500_Ngrid_6144"]# specs = ["L_1024_Ngrid_512"]#, "L_1024_Ngrid_1024"]
Mass_cuts = [1.e12, 5.e12, 1.e13, 5.e13, 7.e13];
L= 500.
ngrid_max = 55
halo_dir = "/mn/stornext/u3/hassanif/neutrino_niayesh/simulations_JD/L_500_Ngrid_6144/"
...
def load_data_bulk(file_path, ngrid, sim, spec, Mass_cut):  ### Loading the bulk velocities
    ...
    """
    cdm_path = f"{file_path}/cdm/output_JD/data_ngrid_{ngrid}_sim_{sim}_{spec}_cdm.pickle"
    nu_path = f"{file_path}/nu/output_JD/data_ngrid_{ngrid}_sim_{sim}_{spec}_nu.pickle" if sim != "0.0ev" else ""
    halo_all_path = f"{file_path}/{convert_mass_cuts([Mass_cut])[0]}/output_JD/data_ngrid_{ngrid}_sim_{sim}_{spec}_halo.pickle"
...
def analyze(Mass_cut, spec, sim):
    ...
    for rank_id in range(rank_total):
        ...
        if sim=="0.0ev":
            ...
            cond = (file_data[6] >= Mass_cut)  # Assuming file_data[6] contains the values you want to compare
            file_data = np.array(file_data).T[cond]
            ...
            mass_conditions = (file_data[6] >= Mass_cut)  # Assuming file_data[6] contains the values you want to compare
    ...
    metadata = {'simulation': sim, 'boxsize': L, 'save_path': save_path, 'N_halos': np.shape(masses)[0], 'N_halos(M>M_cut)/M_tot': np.shape(masses)[0]/(np.shape(masses)[0]), 'Mass cut':f'{Mass_cut:.1e}', '<(v_h - v_bulk(halos).((bias_h + (mass)_h/(1.3e14))^0.85)* (<v(nu)> - v_h^i/2)>': '<y_{h-bulk(h)}.x(nuhi)>', '<(v_h - v_bulk(halos).((bias_h + (mass)_h/(1.3e14))^0.85)* (<v_bulk(nu)> - <v_h>/2))>': '<y_{h-bulk(h)}.x(nuh)>',
